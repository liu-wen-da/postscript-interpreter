...
# MY TESTS ------------------------------------------------------------
    ...
    def testmy_input1(self):
        mytestinput1 = """
            /i -1 def
            /j 1 def
            i j gt { i }{ j } ifelse
        """
        test_case = 'testmy{}'.format(1)
        expr_list = read(mytestinput1)
        for expr in expr_list:
            expr.eval(self.psstacks)
        self.assertEqual(len(self.psstacks.opstack),len(self.opstack_output[test_case]))
        for i in range(0,len(self.opstack_output[test_case])):
    ...
    def testmy_input2(self):
        mytestinput2 = """
            /f { 
                1 dict begin
                /x exch def
                x
                1 1 4 { pop x mul } for 5 mul       
                x -15 mul add 
                x 2 mul sub         
                4 add               
                end } def
            5 f
        """
        test_case = 'testmy{}'.format(2)
        expr_list = read(mytestinput2)
        for expr in expr_list:
            expr.eval(self.psstacks)
        self.assertEqual(len(self.psstacks.opstack),len(self.opstack_output[test_case]))
        for i in range(0,len(self.opstack_output[test_case])):
    ...
    def testmy_input3(self):
        mytestinput3 = """
            ...
            /xcenter
            {
                ...
                pop
                -2 mul
                dx 18 mul 2 mod add dx add
                end
            } def
            (WSU) xcenter
        """
        test_case = 'testmy{}'.format(3)
        expr_list = read(mytestinput3)
        for expr in expr_list:
            expr.eval(self.psstacks)
        self.assertEqual(len(self.psstacks.opstack),len(self.opstack_output[test_case]))
        for i in range(0,len(self.opstack_output[test_case])):
    ...
    def testmy_input4(self):
        mytestinput4 = """
            /first (CptS321 and CptS451) def
            /second (CptS321) def
            /com {  /str exch def
                    dup 4 3 getinterval str 4 3 getinterval eq
                    exch 
                    16 3 getinterval str 4 3 getinterval eq
                 } def
            first second com
        """
        test_case = 'testmy{}'.format(4)
        expr_list = read(mytestinput4)
        for expr in expr_list:
            expr.eval(self.psstacks)
        self.assertEqual(len(self.psstacks.opstack),len(self.opstack_output[test_case]))
        for i in range(0,len(self.opstack_output[test_case])):
    ...
    def testmy_input5(self):
        mytestinput5 = """
            /sum {
                1 1 count 3 sub { pop add } for
            } def
            2 8 9 12 3 sum
        """
        test_case = 'testmy{}'.format(5)
        expr_list = read(mytestinput5)
        for expr in expr_list:
            expr.eval(self.psstacks)
        self.assertEqual(len(self.psstacks.opstack),len(self.opstack_output[test_case]))
        for i in range(0,len(self.opstack_output[test_case])):
