...
class Stacks:
    ...
    def lookup(self,name):
        ...
        if self.scope == 'dynamic':
           for i in range(size):
               if v_name in self.dictstack[size - i - 1][1]:
                   return self.dictstack[size - i - 1][1][v_name]

        elif self.scope == 'static':
            size = self.static_link(name)
            if size != None:
                return self.dictstack[size][1][v_name]
            else:
                return None
            

    def static_link(self,name):
        name = '/' + name
        size = len(self.dictstack) - 1
        while size >= 0:
            if name in self.dictstack[size][1]:
                return size
            size = self.dictstack[size][0]
        return None
    ...
    def stack(self):
        ...
        while size != -1:
            m = size 
            n = self.dictstack[size][0]
            print ("-----", m, "-----", n, "-----")
            for key in self.dictstack[size][1]:
                print(key, " ", self.dictstack[size][1][key])
            size -= 1
        print("-----------------------"+ CEND)
